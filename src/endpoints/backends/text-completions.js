...
router.post('/generate', async function (request, response) {
    ...
    try {
        ...
        if (request.body.api_type === TEXTGEN_TYPES.CHUTES) {
            request.body = _.pickBy(request.body, (_, key) => CHUTES_KEYS.includes(key));
        ...
        if (request.body.api_type === TEXTGEN_TYPES.XAI) {
            ...
            if (request.body.model?.startsWith('grok-3-mini')) {
